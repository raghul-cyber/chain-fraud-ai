...
const { generateAllMockData } = require('./mockDataGenerator');
...
// Generate the mock data once on startup to preserve state
console.log('Generating extensive mock data...');
const mockData = generateAllMockData();
console.log(`Generated ${mockData.suppliers.length} suppliers, ${mockData.invoices.length} invoices, ${mockData.payments.length} payments, and ${mockData.graph.nodes.length} graph nodes.`);
...
// Refresh endpoint in case the user wants to regenerate
app.post('/api/refresh', (req, res) => {
    const newData = generateAllMockData();
    mockData.suppliers = newData.suppliers;
    mockData.invoices = newData.invoices;
    mockData.payments = newData.payments;
    mockData.graph = newData.graph;
    res.json({ message: 'Mock data regenerated successfully!' });
});
